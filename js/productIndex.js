var game = {
	...
	displayProduct: function(){
		...
		for (var i=0; i< 3; i++){
			var j = Math.floor(Math.random() * this.products.length);
			productsText += "<form action='https://www.paypal.com/cgi-bin/webscr' method='post'><div class='card' style='width: 20rem'><img class='card-img-top' src='img/" + j + ".jpg' alt='Card image cap'><div class='card-block'><h4 class='card-title'>" + this.products[j] + "</h4><p class='card-text'>Price: " + this.prices[j] + " Baths</p><input type='hidden' name='cmd' value='_cart' /><input type='hidden' name='add' value='1' /><input type='hidden' name='business' value='payment@nextlevel.com' /><input type='hidden' name='item_name' value='" + this.products[j] + "' /><input type='hidden' name='quantity' value='1' /><input type='hidden' name='amount' value='" + this.prices[j] +"' /><input type='hidden' name='currency_code' value='THB' /><input type='hidden' name='no_note' value='1'><input type='hidden' name='shipping' value='50'><input type='submit' name='submit' value='Add to cart' class='btn btn-primary' /></div></div></form>"
